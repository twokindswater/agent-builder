...
- [ ] (High) 백엔드 프로젝트 초기 설정 - Supabase 설정 및 FastAPI 기본 구조 생성
  - 의존성: 프로젝트 기본 구조 설정 완료
  - 완료 기준: Supabase 데이터베이스 연결 및 기본 API 엔드포인트 실행 가능
  - 산출물: backend/supabase_config.py, backend/app.py, backend/config.py
...
- [ ] (High) 데이터베이스 초기 설정 - Supabase 데이터베이스 설정 및 기본 테이블 스키마 설계
  - 의존성: 백엔드 프로젝트 초기 설정 완료
  - 완료 기준: Supabase에서 users, agents, workflows 테이블 생성 및 연결 테스트 성공
  - 산출물: backend/supabase_schema.sql, backend/models.py
